...
yy='mggl'
if xx==1:
    x, y, x1, y1 = 19, 91, 202, 118
else:
    x, y, x1, y1 = 910, 97, 1279, 145 # 371,601,469,700
...
if xx==1:
    ...
    img1 = cv2.threshold(_crop_img.copy(), 240, 255, cv2.THRESH_BINARY|cv2.THRESH_OTSU)[1]
    ...
    img1 = cv2.threshold(_crop_img.copy(), 0, 255, cv2.THRESH_BINARY_INV|cv2.THRESH_OTSU)[1]  # 转换为二值图像, thresh=63
    img1 = cv2.morphologyEx(img1, cv2.MORPH_CLOSE, sqKernel)
    res_cont = cv2.findContours(img1, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)[0]
# print(len(res_cont))
ori_list = []
...
for _row in range(len(ori_list)):
    ...
    _re = cv2.matchTemplate(
        ...
        numpy.load(OT.mapnpy('xt_147_e')), method=cv2.TM_CCOEFF_NORMED)
    print(_re)
